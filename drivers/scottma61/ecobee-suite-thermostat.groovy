 ...
 *	1.4.1  - Changed @mattw01's version as follows: parent.settings. --> parent. ; toDouble() --> toFloat() on line 1126  ; added hard brackets on line 706
 */

def getVersionNum() { return "1.4.1" }
private def getVersionLabel() { return "Ecobee Suite Thermostat, version ${getVersionNum()}" }
import groovy.json.JsonSlurper
 
metadata {
	definition (name: "Ecobee Suite Thermostat", namespace: "Scottma61", author: "Barry A. Burke (storageanarchy@gmail.com)") {
		capability "Actuator"
...
def generateEvent(Map results) {
    ...
    //LOG("Debug level of parent: ${parent.debugLevel}", 4, null, "debug")
...
private def usingSmartAuto() {
    ...
    if (parent.smartAuto) { return parent.smartAuto }
    return false
}
...
def whatHoldType() {
    ...
    switch (myHoldType) {
        ...
        case 'Hourly':
            ...
            } else if ((parent.holdType == 'Specified Hours') && parent.holdHours && parent.holdHours.isNumber()) {
            	sendHoldType = parent.holdHours
            } else if ( parent.holdType == '2 Hours') {
            	sendHoldType = 2
            } else if ( parent.holdType == '4 Hours') {
            	sendHoldType = 4            
            } else {
            	sendHoldType = 2
            }
        ...
        case null :		// null means use parent value
            theHoldType = 'Parent'
        case 'Parent':
            parentHoldType = parent.holdType
			...
			if (theHoldType == 'Thermostat') {
                ...
                switch (parentHoldType) {
                    ...
                    case 'Specified Hours':
                        if (parent.holdHours && parent.holdHours.isNumber()) {sendHoldType = parent.holdHours} else {sendHoldType = 2}
                        break;
...
private Integer howManyHours() {
    ...
    } else if (parent.holdHours && (parent.holdHours.isNumber())) {
    	sendHoldHours = parent.holdHours.toInteger()
...
private debugLevel(level=3) {
    // TODO: Fix parent.debugLevel
    return 3
	Integer dbg = device.currentValue('debugLevel')?.toInteger()
	Integer debugLvlNum = dbg ? dbg : (parent.debugLevel ? parent.debugLevel.toInteger() : 3)
