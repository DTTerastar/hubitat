...
def zwaveEvent(hubitat.zwave.commands.crc16encapv1.Crc16Encap cmd) {
	log.trace("zwaveEvent(): CRC-16 Encapsulation Command received: ${cmd}")
...
def zwaveEvent(hubitat.zwave.commands.sensorbinaryv2.SensorBinaryReport cmd) {
        ...
        switch (cmd.sensorType) {
            case 1:
                ...
                def value = Math.round(convertTemperatureIfNeeded(cmd.scaledSensorValue, cmdScale,  cmd.precision).toFloat()).toString()
                def unit  = getTemperatureScale()

                return createEvent(name:"temperature", value: value, unit: unit)
            case 3:
                def value = cmd.scaledSensorValue.toInteger().toString()

                return createEvent(name:"illuminance", value: value, unit: "lux")
        }
}
