...
def parse(String description) {
	...
	if (!map) {
		if (description?.startsWith('zone status')) {
			...
			map = (zs.isAlarm1Set() || zs.isAlarm2Set()) ? getMotionResult('active') : getMotionResult('inactive')
		} else {
			...
			if (descMap?.clusterInt == 0x0001 && descMap.attrInt == 0x0020 && descMap.commandInt != 0x07 && descMap?.value) {
				...
				logDebug "Doing a read attr motion event"
	...
	if (map && map.descriptionText) log.info(map.descriptionText)
...
private Map getBatteryResult(rawValue) {
	logDebug "Battery rawValue = ${rawValue}"
