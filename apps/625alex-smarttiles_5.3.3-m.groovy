...
def command() {
	logDebug "command received with params $params"
	...
	if (type == "thermostatHeat" || type == "thermostatCool") {
        ...
        logDebug "executing Hello Home '$value'"
...
def position() {
	...
	state.sortOrder = map
	logDebug "state.sortOrder: $state.sortOrder"
}
...
def generateURL(path) {
	logDebug "resetOauth: $settings.resetOauth, $resetOauth, $settings.resetOauth"
	if (settings.resetOauth) {
		logDebug "Reseting Access Token"
...
def handler(e) {
	logDebug "event from: $e.displayName, value: $e.value, source: $e.source, description: $e.description"
	updateStateTS()
}

def updateStateTS() {
	logDebug "updating TS"
	state.ts = now()
}
