...
def refresh() {
    if (isActive) {
        ...
        if (state.primaryMins>0 || state.secondaryMins>0 || state.contactMins>0 || state.switchMins>0) {
            logDebug "${app.label} not turning off (primary=${state.primaryMin?:0}m secondary=${state.secondaryMins?:0}m contacts=${state.contactMins?:0}m switches=${state.switchMins?:0}m)"
            return;
        }
        ...
        if (state.primaryMins>0 || state.contactMins>0 || state.switchMins>0) {
            logDebug "${app.label} turning ON (primary=${state.primaryMin?:0}m contacts=${state.contactMins?:0}m switches=${state.switchMins?:0}m)"
